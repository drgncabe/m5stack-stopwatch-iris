...
- Web precise-setting controls with unsaved-change indication and cancel behavior
- Web Apps page and Apps API backed by the Iris app registry
- Responsive web navigation with a collapsed mobile menu
- Device and hardware diagnostics with heap, PSRAM, and firmware partition usage
- Per-app foreground update hints for better CPU/display power behavior
- PowerManager resource requests for future apps needing performance, display, or WiFi
...
The Badge page can upload one active PNG, JPEG, or GIF under 4 MB to the StopWatch filesystem. PNG, JPEG, and animated GIF files render in the Badge app. Upload validation reports specific errors for unsupported formats, empty uploads, oversized files, invalid image data, and insufficient badge storage.
...
Hardware Diagnostics includes a BLE page for inspecting status, active BLE HID profile, active/bonded devices, starting or stopping advertising, disconnecting the active host, and clearing stored bonds.
