...
bool isSettingsPage(const String& page) {
...
bool isDevelopmentPage(const String& page) {
  return page == "development" || page == "apps";
}

const char* activePrimaryPage(const String& page) {
  if (isSettingsPage(page)) return "settings";
  if (isDevelopmentPage(page)) return "development";
...
void WifiService::configurePortalRoutes() {
  ...
  server_.on("/apps", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/development/apps", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/badge", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/badge", true);
    server_.send(302, "text/plain", "");
  });
  ...
  server_.on("/settings/display", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=display", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/theme", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=theme", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/badge", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=badge", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/datetime", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=datetime", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/touch", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=touch", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/sound", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/?page=sound", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/settings/wifi", HTTP_GET, [this]() {
  ...
  server_.on("/display", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/display", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/theme", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/theme", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/datetime", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/datetime", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/touch", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/touch", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/sound", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/sound", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/wifi", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/wifi", true);
    server_.send(302, "text/plain", "");
  });
  server_.on("/power", HTTP_GET, [this]() {
    server_.sendHeader("Location", "/settings/power", true);
    server_.send(302, "text/plain", "");
  });
  ...
  server_.on("/development/apps", HTTP_GET, [this]() {
...
void WifiService::handleControlPanel() {
  ...
  if (page == "settings") {
    html += F("<section><h2>Settings</h2><p class='hint'>Normal device configuration lives here. Diagnostics and raw hardware tools stay under Development.</p><div class='settings-grid'>");
    appendSettingsCard(html, "/settings/display", "Display", "Brightness, timeout, dimming, and rotation");
    appendSettingsCard(html, "/settings/theme", "Theme & Widgets", "Watch-face appearance, layout, and complications");
    appendSettingsCard(html, "/settings/badge", "Badge", "Badge image upload, display mode, and keep-awake behavior");
    appendSettingsCard(html, "/settings/datetime", "Date & Time", "Locale, formats, timezone, NTP sync, and manual time");
    appendSettingsCard(html, "/settings/touch", "Touch", "Touch delay and selection feel");
    ...
    html += F(" invalid</span></p></div><h3>Debug</h3><div class='facts'>");
    html += F("<p><b>ESP-NOW channel</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar active channel"));
    html += F("</span></p><p><b>Receiver initialized</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar receiver initialized"));
    html += F("</span></p><p><b>Raw RX count</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar raw RX count"));
    html += F("</span></p><p><b>Last sender MAC</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar last sender MAC"));
    html += F("</span></p><p><b>Last packet length</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar last packet length"));
    html += F("</span></p><p><b>Last drop reason</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar last drop reason"));
    html += F("</span></p><p><b>Drop counts</b><span>");
    html += F("len ");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar wrong length drops"));
    html += F(" / magic ");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar bad magic drops"));
    html += F(" / ver ");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar bad version drops"));
    html += F(" / type ");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar bad type drops"));
    html += F(" / crc ");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar bad CRC drops"));
    html += F("</span></p><p><b>Seconds since valid</b><span>");
    html += escapeHtml(snapshotValue(snapshot, "Ragnar seconds since valid packet"));
    html += F("</span></p></div>");
    ...
    html += F("</span></p><p><b>Minimum heap</b><span>");
    html += formatBytes(ESP.getMinFreeHeap());
    ...
    html += F("</span></p><p><b>Sketch used</b><span>");
    html += formatBytes(ESP.getSketchSize());
    html += F("</span></p><p><b>Sketch free</b><span>");
    html += formatBytes(ESP.getFreeSketchSpace());
    html += F("</span></p><p><b>PSRAM</b><span>");
    html += formatBytes(ESP.getFreePsram());
    html += F(" free / ");
    ...
    html += F("<a class='button' href='/development/apps'>Apps Registry</a>");
...
void WifiService::handleApiDeviceInfo() {
  ...
  json += F("\",\"minFreeHeapBytes\":");
  json += String(ESP.getMinFreeHeap());
  json += F(",\"minFreeHeap\":\"");
  json += formatBytes(ESP.getMinFreeHeap());
  json += F("\",\"flashBytes\":");
  ...
  json += F(",\"sketch\":\"");
  json += formatBytes(ESP.getSketchSize());
  json += F("\",\"freeSketchBytes\":");
  json += String(ESP.getFreeSketchSpace());
  json += F(",\"freeSketch\":\"");
  json += formatBytes(ESP.getFreeSketchSpace());
  json += F("\",\"psramBytes\":");
  json += String(ESP.getPsramSize());
  json += F(",\"psram\":\"");
  json += formatBytes(ESP.getPsramSize());
  json += F("\",\"freePsramBytes\":");
  json += String(ESP.getFreePsram());
  json += F(",\"freePsram\":\"");
  json += formatBytes(ESP.getFreePsram());
  json += F("\",\"battery\":\"");
  json += escapeJson(snapshotValue(snapshot, "Battery"));
  json += F("\",\"displayPower\":\"");
  json += escapeJson(snapshotValue(snapshot, "Display power"));
...
void WifiService::handleApiRagnar() {
  ...
  json += F("\",\"receiverInitialized\":");
  json += snapshotValue(snapshot, "Ragnar receiver initialized") == "Yes" ? String("true")
                                                                           : String("false");
  json += F(",\"rawRxCount\":");
  json += String(snapshotInt(snapshot, "Ragnar raw RX count", 0));
  json += F(",\"lastSenderMac\":\"");
  json += escapeJson(snapshotValue(snapshot, "Ragnar last sender MAC"));
  json += F("\",\"lastPacketLength\":");
  json += String(snapshotInt(snapshot, "Ragnar last packet length", 0));
  json += F(",\"lastDropReason\":\"");
  json += escapeJson(snapshotValue(snapshot, "Ragnar last drop reason"));
  json += F("\",\"dropCounts\":{\"wrongLength\":");
  json += String(snapshotInt(snapshot, "Ragnar wrong length drops", 0));
  json += F(",\"badMagic\":");
  json += String(snapshotInt(snapshot, "Ragnar bad magic drops", 0));
  json += F(",\"badVersion\":");
  json += String(snapshotInt(snapshot, "Ragnar bad version drops", 0));
  json += F(",\"badType\":");
  json += String(snapshotInt(snapshot, "Ragnar bad type drops", 0));
  json += F(",\"badCrc\":");
  json += String(snapshotInt(snapshot, "Ragnar bad CRC drops", 0));
  json += F("},\"secondsSinceLastValid\":\"");
  json += escapeJson(snapshotValue(snapshot, "Ragnar seconds since valid packet"));
  json += F("\",\"validPackets\":");
...
void WifiService::appendNavigation(String& html, const String& page) {
  ...
  constexpr const char* hrefs[] = {"/dashboard", "/device", "/settings", "/development"};
  const char* active = activePrimaryPage(page);
  html += F("<nav class='desktop-nav'>");
  for (size_t i = 0; i < sizeof(pages) / sizeof(pages[0]); ++i) {
    html += F("<a class='nav");
    if (strcmp(active, pages[i]) == 0) html += F(" active");
    html += F("' href='");
    html += hrefs[i];
    html += F("'>");
    html += labels[i];
...
void WifiService::appendSettingsCard(String& html, const char* href, const char* title,
                                     const char* detail) {
  html += F("<a class='settings-card' href='");
  html += href;
  html += F("'><b>");
  html += title;
  html += F("</b><span>");
...
void WifiService::appendBluetoothPage(String& html, const String& snapshot) {
  ...
  html += F("</span></p><p><b>Profile</b><span>");
  html += escapeHtml(snapshotValue(snapshot, "BLE profile"));
  ...
  html += escapeHtml(snapshotValue(snapshot, "BLE bonded list"));
  html += F("</span></p></div>");
